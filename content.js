...
waitForSeriesLinks()
  .then((links) => {
    ...
    checkNextSeries(links, 0); // Starte Prüfung mit Index 0
  })
  .catch((err) => {
...
function checkNextSeries(links, index) {
  ...
  // Erstelle unsichtbares iframe im Hintergrund
  const iframe = document.createElement("iframe");
  iframe.style.display = "none"; // Unsichtbar
  iframe.src = url;
  ...
  const interval = setInterval(() => {
    ...
    browser.runtime.sendMessage({ type: "get-language-status", seriesId }).then((res) => {
      if (res.hasLanguage === null) return; // noch keine Antwort → weiter warten
